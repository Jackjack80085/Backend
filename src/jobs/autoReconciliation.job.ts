...
export async function autoReconciliationJob() {
  const logger = new JobLogger('autoReconciliation')
  ...
  try {
    ...
    for (const wallet of wallets) {
      ...
      const ledgerSum = ledgerEntries.reduce((sum: bigint, entry: any) => sum + BigInt(entry.amount.toString()), 0n)
