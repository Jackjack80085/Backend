...
router.post('/', async (req, res) => {
    try {
        ...
        const existingWT = await database_1.default.walletTransaction.findUnique({ where: { paywiseTxnId } });
        if (existingWT) {
        ...
        if (event === 'payment.success' || status === 'success') {
            ...
            try {
                await (0, payment_service_1.failPayment)({
                    transactionId: order_id,
        ...
        const freshTxn = await database_1.default.transaction.findUnique({
            where: { id: order_id },
            include: { partner: true },
        });
        if (freshTxn) {
            partnerWebhook_service_1.default.notifyPaymentStatus(freshTxn).catch((err) => {
